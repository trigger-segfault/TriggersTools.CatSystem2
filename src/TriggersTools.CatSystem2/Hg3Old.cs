...
namespace TriggersTools.CatSystem2 {
	...
	/// </summary>
	[JsonObject]
	public sealed partial class Hg3Old : IReadOnlyCollection<Hg3OldImage> {
		#region Constants
		...
		/// </summary>
		[JsonIgnore]
		private IReadOnlyList<Hg3OldImage> images;
		...
		/// </summary>
		[JsonProperty("images")]
		public IReadOnlyList<Hg3OldImage> Images {
			get => images;
			private set {
				images = value;
		...
		/// <summary>
		///  Gets the name of the file for loading the <see cref="Hg3Old"/> data.
		/// </summary>
		[JsonIgnore]
		...
		private Hg3Old(string fileName, HGXHDR hdr, HG3STDINFO[] stdInfos, long[][] frameOffsets, bool expand) {
			...
			Expanded = expand;
			Hg3OldImage[] images = new Hg3OldImage[stdInfos.Length];
			for (int i = 0; i < images.Length; i++)
				images[i] = new Hg3OldImage(i, stdInfos[i], frameOffsets[i], this);
		...
		/// <param name="stdInfos">The HG3STDINFO struct array containing image dimension information.</param>
		/// <param name="expand">True if the images were extracted while <paramref name="expand"/> was true.</param>
		private Hg3Old(string fileName, HGXHDR hdr, HG3STDINFO[] stdInfos, bool expand) {
			Version = CurrentVersion;
			FileName = Path.GetFileName(fileName);
			Unknown = hdr.Type;
			//Unknown3 = hdr.Unknown3;
			...
			Hg3OldImage[] images = new Hg3OldImage[stdInfos.Length];
			for (int i = 0; i < images.Length; i++)
				images[i] = new Hg3OldImage(i, stdInfos[i], this);
			this.images = Array.AsReadOnly(images);
		}
		...
		/// </summary>
		/// <param name="imgIndex">
		///  The first index, which is assocaited to an <see cref="Hg3Old.ImageIndex"/>.
		/// </param>
		/// <param name="frmIndex">
		///  The second index, which is associated to a frame inside an <see cref="Hg3OldImage"/>.
		/// </param>
		...
		/// </summary>
		/// <param name="directory">The directory of the <see cref="Hg3Old"/> images.</param>
		/// <param name="imgIndex">
		///  The first index, which is assocaited to an <see cref="Hg3Old.ImageIndex"/>.
		/// </param>
		/// <param name="frmIndex">
		...
		/// </summary>
		/// <param name="directory">The directory of the <see cref="Hg3Old"/>.</param>
		/// <param name="fileName">The base file name of the <see cref="Hg3Old"/>.</param>
		...
		/// </exception>
		public static Hg3Old FromJsonDirectory(string directory, string fileName) {
			if (directory == null)
				throw new ArgumentNullException(nameof(directory));
			if (fileName == null)
				throw new ArgumentNullException(nameof(fileName));
			string jsonFile = Path.Combine(directory, GetJsonFileName(fileName));
			return JsonConvert.DeserializeObject<Hg3Old>(File.ReadAllText(jsonFile));
		}
		/// <summary>
		...
		public IEnumerator<Hg3OldImage> GetEnumerator() => Images.GetEnumerator();
