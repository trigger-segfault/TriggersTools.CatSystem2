...
namespace TriggersTools.CatSystem2 {
	partial class Hg3Old {
		#region Extract

		private static Hg3Old Extract(Stream stream, string fileName, string directory, bool saveFrames, bool expand) {
			BinaryReader reader = new BinaryReader(stream);
			HGXHDR hdr = reader.ReadUnmanaged<HGXHDR>();

			UnexpectedFileTypeException.ThrowIfInvalid(hdr.Signature, HGXHDR.ExpectedHg3Signature);
			...
			do {
				...
				stream.Position = startPosition + offsetNext;
			} while (offsetNext != 0);
			...
			Hg3Old hg3 = new Hg3Old(Path.GetFileName(fileName), hdr, stdInfos, allFrameOffsets, saveFrames && expand);
			...
			if (saveFrames) {
				for (int imgIndex = 0; imgIndex < hg3.Count; imgIndex++) {
					...
					Hg3OldImage hg3Image = hg3.Images[imgIndex];
		...
		private static void ExtractImage(BinaryReader reader, Hg3FrameInfo atomInfo, bool expand, string pngFile) {
			...
			//ZLib1.Uncompress(bufferTmp2, ref img.OriginalDataLength, bufferTmp, bufferTmp.Length);
			//ZLib1.Uncompress(cmdBufferTmp2, ref img.OriginalCmdLength, cmdBufferTmp, cmdBufferTmp.Length);
