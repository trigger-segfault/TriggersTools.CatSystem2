...
namespace TriggersTools.CatSystem2 {
	...
	public sealed partial class CstScene : IReadOnlyCollection<ISceneLine> {
		...
		private IReadOnlyList<SceneLine> IOLines {
			get {
				SceneLine[] lines = new SceneLine[Count];
				for (int i = 0; i < Count; i++)
					lines[i] = new SceneLine(Lines[i]);
			...
			set {
				...
				for (int i = 0; i < value.Count; i++)
					newLines[i] = SceneUtils.CreateLine(value[i].Type, value[i].Content);
		...
		public CstScene(string fileName, IEnumerable<ISceneLine> lines) {
			FileName = fileName;
			Lines = lines.ToImmutableArrayLW();
		}
