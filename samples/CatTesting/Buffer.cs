...
namespace TriggersTools.CatSystem2.Testing {
	internal static class Buffer451 {
		...
		internal unsafe static void Memmove(byte* dest, byte* src, long _len) {
			uint len = (uint) _len;
		...
		PInvoke:
			...
			//_Memmove(dest, src, new IntPtr(len));
		}
		...
		private unsafe static void _Memmove(byte* dest, byte* src, IntPtr len) {
		...
		[ResourceExposure(ResourceScope.None)]
		[ReliabilityContract(Consistency.WillNotCorruptState, Cer.Success)]
		extern private unsafe static void __Memmove(byte* dest, byte* src, IntPtr len);*/
